...
// Pin configuration for trigger
const int BUTTON_PIN = 0; // Boot button on ESP32
int lastButtonState = HIGH;
bool ledState = false; // Track local state of target D4 LED
...
<head>
  ...
  <title>ESP32 Wi-Fi Config Portal</title>
...
<body>
  <div class="card">
    ...
    <p>Select your home Wi-Fi network and enter the password to connect the ESP32.</p>
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>ESP32 Master IoT Control Center</title>
  <link href="https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;600;800&display=swap" rel="stylesheet">
  <style>
    :root {
      --bg: #090d16;
...
<body>
  <div class="container">
    ...
    <div class="subtitle" style="text-align: left; margin-bottom: 20px;">ESP32 Master Web Controller (MQTT Cloud)</div>
    ...
    <div class="board-selector">
      <button class="board-btn active" id="btn-all" onclick="selectBoard(0, 'btn-all')">All Boards</button>
      <button class="board-btn" id="btn-b1" onclick="selectBoard(1, 'btn-b1')">ESP8266 (B1)</button>
      <button class="board-btn" id="btn-b2" onclick="selectBoard(3, 'btn-b2')">Raspberry Pi (B3)</button>
    </div>
  ...
  <div class="footer">Hosted on ESP32 Controller</div>
  ...
  <div id="wifi-modal" class="modal">
    <div class="modal-content">
      ...
      <p style="color: var(--text-muted); font-size: 0.85rem; margin-bottom: 20px;">Change ESP32 home Wi-Fi settings</p>
  ...
  <script>
    let currentBoard = 0;
    const pinsConfig = [
      { name: "D0", desc: "GPIO 16 / Aux Relay" },
      { name: "D1", desc: "GPIO 5 / Water Pump" },
      { name: "D2", desc: "GPIO 4 / Solenoid Valve" },
      { name: "D3", desc: "GPIO 0 / Indicator LED" },
      { name: "D4", desc: "GPIO 2 / Onboard LED" }
    ];

    function selectBoard(id, btnId) {
      currentBoard = id;
      ...
      fetch(`/savewifi?ssid=${encodeURIComponent(ssid)}&password=${encodeURIComponent(password)}`)
        .then(res => {
          if (res.ok) {
            ...
            alert('Settings saved. ESP32 will restart to connect to new network.');
          } else {
            throw new Error();
          }
        })
...
void sendEspNowCommand(int board_id, const char* pin, const char* action, int speed) {
  // Format payload as comma-separated string: "board_id,pin,action,speed"
  char payload[64];
  snprintf(payload, sizeof(payload), "%d,%s,%s,%d", board_id, pin, action, speed);
  
  Serial.printf("Publishing MQTT message: %s\n", payload);
  
  if (WiFi.status() == WL_CONNECTED) {
    if (!client.connected()) {
      Serial.print("Reconnecting to MQTT Broker for publish...");
    ...
    if (client.connected()) {
      ...
      Serial.println(success ? "MQTT Publish Success" : "MQTT Publish Fail");
    } else {
      Serial.println("MQTT Not Connected - Command Dropped");
    }
  } else {
    Serial.println("WiFi Not Connected - Command Dropped");
  }
}
...
void handleSetPin() {
  if (server.hasArg("board") && server.hasArg("pin") && server.hasArg("state")) {
    int board = server.arg("board").toInt();
    String pinStr = server.arg("pin");
    String stateStr = server.arg("state");
    stateStr.toUpperCase();
    
    sendEspNowCommand(board, pinStr.c_str(), stateStr == "ON" ? "ON" : "OFF", 0);
    server.send(200, "text/plain", "OK");
  } else {
    server.send(400, "text/plain", "Bad Request");
  }
...
void handleSetBlink() {
  if (server.hasArg("board") && server.hasArg("pin") && server.hasArg("enabled")) {
    int board = server.arg("board").toInt();
    String pinStr = server.arg("pin");
    ...
    if (enabled) {
      sendEspNowCommand(board, pinStr.c_str(), "BLINK", speed);
    } else {
      sendEspNowCommand(board, pinStr.c_str(), "STOPBLINK", 0);
    }
    server.send(200, "text/plain", "OK");
  } else {
    server.send(400, "text/plain", "Bad Request");
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  Serial.printf("Received MQTT data on [%s]: %s\n", topic, message.c_str());
  
  if (String(topic) == mqtt_sensor_topic) {
    int commaIdx = message.indexOf(',');
    if (commaIdx != -1) {
      String temp = message.substring(0, commaIdx);
      String hum = message.substring(commaIdx + 1);
      Serial.printf(">>> Real-Time Pi 3 Sensor: Temp = %s°C, Hum = %s%%\n", temp.c_str(), hum.c_str());
    }
  }
}
...
void setup() {
  ...
  preferences.begin("wifi-config", false);
  ssid = preferences.getString("ssid", "PIXELBOX T1");
  password = preferences.getString("password", "Pixelbox2022");
  preferences.end();

  Serial.println("Attempting to connect to Wi-Fi SSID: " + ssid);

  // Try to connect to Wi-Fi in Station mode
  WiFi.mode(WIFI_STA);
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // Setup mDNS
    if (MDNS.begin("esp32")) {
      Serial.println("mDNS responder started. Reachable at: http://esp32.local");
    } else {
    ...
    WiFi.mode(WIFI_AP);
    // SoftAP SSID: ESP32_IoT_Config, Password: no password for ease of connection (or 12345678)
    WiFi.softAP("ESP32_IoT_Config", "12345678");
...
void loop() {
  ...
  // 1. Read BOOT button (GPIO 0)
  int buttonState = digitalRead(BUTTON_PIN);
  if (buttonState == LOW && lastButtonState == HIGH) {
    delay(50); // Debounce
    if (digitalRead(BUTTON_PIN) == LOW) {
      ledState = !ledState;
      Serial.printf("BOOT button pressed. Toggling ALL ESP8266 LEDs to %s\n", ledState ? "ON" : "OFF");
      sendEspNowCommand(0, "D4", ledState ? "ON" : "OFF", 0);
    }
  }
  lastButtonState = buttonState;

  // 2. Read Serial input commands
  if (Serial.available() > 0) {
    ...
    if (input.equalsIgnoreCase("on")) {
      ledState = true;
      sendEspNowCommand(0, "D4", "ON", 0);
    } else if (input.equalsIgnoreCase("off")) {
      ledState = false;
      sendEspNowCommand(0, "D4", "OFF", 0);
    } else if (input.equalsIgnoreCase("blink")) {
      sendEspNowCommand(0, "D4", "BLINK", 250);
    } else if (input.equalsIgnoreCase("stopblink")) {
      sendEspNowCommand(0, "D4", "STOPBLINK", 0);
    } else if (input.startsWith("setpin ")) {
      // Format: "setpin [board_id] <pin> <on/off>" -> e.g. "setpin 1 D0 ON" or "setpin D0 ON"
      ...
      if (space1 != -1 && space2 != -1 && space3 != -1) {
        targetBoard = input.substring(space1 + 1, space2).toInt();
        pinName = input.substring(space2 + 1, space3);
        stateVal = input.substring(space3 + 1);
      } else if (space1 != -1 && space2 != -1) {
        pinName = input.substring(space1 + 1, space2);
        stateVal = input.substring(space2 + 1);
      }
      
      if (!pinName.isEmpty()) {
        pinName.toUpperCase();
        stateVal.toUpperCase();
        sendEspNowCommand(targetBoard, pinName.c_str(), stateVal.c_str(), 0);
      } else {
        Serial.println("Invalid format. Use: 'setpin [board_id] <pin> <ON/OFF>'");
      }
    } else if (input.startsWith("setblink ")) {
      // Format: "setblink [board_id] <pin> <speed_ms>" -> e.g. "setblink 2 D0 500" or "setblink D0 500"
      int space1 = input.indexOf(' ');
      int space2 = input.indexOf(' ', space1 + 1);
      int space3 = input.indexOf(' ', space2 + 1);
      ...
      if (space1 != -1 && space2 != -1 && space3 != -1) {
        targetBoard = input.substring(space1 + 1, space2).toInt();
        pinName = input.substring(space2 + 1, space3);
        speedVal = input.substring(space3 + 1).toInt();
        valid = true;
      } else if (space1 != -1 && space2 != -1) {
        pinName = input.substring(space1 + 1, space2);
        speedVal = input.substring(space2 + 1).toInt();
        valid = true;
      }
      
      if (valid && !pinName.isEmpty()) {
        pinName.toUpperCase();
        sendEspNowCommand(targetBoard, pinName.c_str(), "BLINK", speedVal);
      } else {
        Serial.println("Invalid format. Use: 'setblink [board_id] <pin> <speed_ms>'");
      }
    } else if (input.startsWith("setstopblink ")) {
      // Format: "setstopblink [board_id] <pin>" -> e.g. "setstopblink 1 D0" or "setstopblink D0"
      int space1 = input.indexOf(' ');
      int space2 = input.indexOf(' ', space1 + 1);
      ...
      if (space1 != -1 && space2 != -1) {
        targetBoard = input.substring(space1 + 1, space2).toInt();
        pinName = input.substring(space2 + 1);
      } else if (space1 != -1) {
        pinName = input.substring(space1 + 1);
      }
      
      if (!pinName.isEmpty()) {
        pinName.toUpperCase();
        sendEspNowCommand(targetBoard, pinName.c_str(), "STOPBLINK", 0);
      } else {
        Serial.println("Invalid format. Use: 'setstopblink [board_id] <pin>'");
      }
    } else if (input != "") {
      Serial.println("Unknown command. Supported: 'on', 'off', 'blink', 'stopblink', 'setpin [board_id] <pin> <ON/OFF>', 'setblink [board_id] <pin> <speed_ms>', 'setstopblink [board_id] <pin>'");
    }
  }
